					...
					function() {
					    ...
					    $
							...
							success : function(
								data,
								textStatus) {
							    var newlist = '<li><button type="button" value="'
								    + data.BOARD_NUM
								    + '" class="selectboard" onclick="select(this)">'
								    + name
								    + '</button></li>'
							    $('#boardList')
								    .append(
					...
					<form action="update" method="post">
						<ul class="ulbox">
							...
							<li><div class='desc'>가입방식</div>
								<div class="inputbox">
									<input type="radio" name="group_joinmanner" value="free" checked="checked"> 가입 신청시 바로 가입가능 <input type="radio" name="group_joinmanner" value="need"> 가입 신청후 승인 필요
								</div></li>
							<li><div class="desc">카페아이콘</div> <img id="imgpreview" alt="" src="${contextPath}/resources/image/${result.GROUP_ICON}"> <button id="imgbtn" type="button">업로드</button></li>
							<li><div style="text-align: right;">
									<button id="submitbtn">확인</button>
								</div></li>
						</ul>
						<input type="hidden" name="group_num" value="${result.GROUP_NUM}"> <input type="hidden" id="group_icon" name="group_icon" value="${result.GROUP_ICON}">
					</form>
				</div>
			</div>
			<div class="divbox">
					...
					<div id="tablebox">
						<ul id="boardList">
							...
							<c:forEach var="list" items="${boardList}">
								<li>
									<button type="button" class="selectboard" id="list${i}" value="${list.BOARD_NUM}" onclick="select(this)">${list.BOARD_NAME}</button>
								</li>
						...
						<c:forEach var="list" items="${memberList.member}">
							<c:if test="${list.G_LIST_STATE ne 'deport'}">
								<tr>
									...
									<td><c:if test="${list.G_LIST_STATE ne 'manager'}">
											<form action="deport" method="post" class="inlineform">
												<input type="hidden" name="group_num" value="${result.GROUP_NUM}"> <input type="hidden" name="user_id" value="${list.USER_ID}">
												<button class="memberbtn" value="${list.USER_ID}">추방</button>
											</form>
											<form action="yield" method="post" class="inlineform">
												<input type="hidden" name="group_num" value="${result.GROUP_NUM}"> <input type="hidden" name="user_id" value="${list.USER_ID}">
												<button class="memberbtn">관리자 양도</button>
											</form>
										</c:if></td>
								</tr>
							</c:if>
							...
							<tr>
								...
								<td>
									<form action="apply" class="inlineform">
										<input type="hidden" name="group_num" value="${result.GROUP_NUM}"> <input type="hidden" name="user_id" value="${list.USER_ID}"> <input type="hidden" name="apply" value="true">
										<button class="memberbtn">승낙</button>
									</form>
									<form action="apply" class="inlineform">
										<input type="hidden" name="group_num" value="${result.GROUP_NUM}"> <input type="hidden" name="user_id" value="${list.USER_ID}"> <input type="hidden" name="apply" value="false">
